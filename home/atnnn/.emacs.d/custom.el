(custom-set-variables
 ;; custom-set-variables was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(Info-additional-directory-list (quote ("/home/atnnn/code/ciao/build/doc/")))
 '(Info-default-directory-list (quote ("/usr/share/info/emacs-24/" "/usr/share/info/")))
 '(backup-directory-alist (quote ((".*" . "~/.emacs.d/backups/"))))
 '(battery-mode-line-format "[%b%p%%]")
 '(blink-cursor-mode nil)
 '(c-auto-align-backslashes nil)
 '(c-offsets-alist (quote ((innamespace . 0))))
 '(column-number-mode t)
 '(default-input-method "rfc1345")
 '(desktop-save-mode t)
 '(display-battery-mode t)
 '(fringe-mode (quote (2 . 6)) nil (fringe))
 '(global-font-lock-mode t)
 '(global-highlight-sexp-mode nil)
 '(highlight-symbol-idle-delay 0.1)
 '(hippie-expand-try-functions-list
   (quote
    (try-complete-file-name-partially try-complete-file-name try-expand-all-abbrevs try-expand-dabbrev try-expand-dabbrev-all-buffers try-expand-dabbrev-from-kill try-complete-lisp-symbol-partially try-complete-lisp-symbol)))
 '(hl-sexp-background-color "#fffced")
 '(ido-mode t nil (ido))
 '(indent-tabs-mode nil)
 '(indicate-empty-lines t)
 '(inhibit-startup-screen t)
 '(menu-bar-mode nil)
 '(mouse-autoselect-window t)
 '(package-selected-packages
   (quote
    (crosshairs rainbow-identifiers rainbow-delimiters nav-flash idle-highlight highlight-tail highlight-symbol highlight-sexp highlight-numbers diff-hl context-coloring col-highlight php-extras php+-mode php-mode nix-mode haskell-mode flycheck)))
 '(save-place t nil (saveplace))
 '(savehist-mode t)
 '(scroll-bar-mode nil)
 '(scroll-preserve-screen-position t)
 '(select-enable-primary t)
 '(show-paren-mode t)
 '(size-indication-mode t)
 '(tool-bar-mode nil)
 '(use-file-dialog nil))
(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(highlight-symbol-face ((t (:underline t))))
 '(mode-line ((t (:background "gainsboro" :foreground "black" :inverse-video t))))
 '(mode-line-inactive ((t (:inherit mode-line :background "gray" :foreground "grey20" :box (:line-width -1 :color "grey75") :weight light))))
 '(whitespace-line ((t (:foreground "tomato"))))
 '(whitespace-trailing ((t (:background "red1" :foreground "yellow")))))
